...
function stalenessClass(days?: number | null) {
...
function UserDetailModal({ userId, onClose }: { userId: string; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE}/admin/users/${userId}/details`, { headers: { Authorization: `Bearer ${token}` } })
      .then((r) => r.json())
      .then((d) => {
        if (cancelled) return;
        if (d.success) setDetail(d.data);
        else setError(d.message || "Failed to load user details.");
      })
      .catch(() => { if (!cancelled) setError("Cannot reach the server."); })
...
function Users() {
  ...
  }, [page, sortKey, sortDir, debouncedSearch, roleFilter, statusFilter, activityFilter]);
  ...
  return (
    <>
      ...
      <div className={TOOLBAR}>
        ...
        <select className={FILTER_SELECT} value={activityFilter} onChange={(e) => { setActivityFilter(e.target.value); setPage(1); }}>
